 ...
 *
 *         ── подсветка строки гаснет НА ВРЕМЯ ПРОКРУТКИ ──
 *         Пока лента едет, курсор стоит на месте, а строки идут под ним:
 *         наведённая меняется каждый кадр, и каждый кадр платится пересчётом
 *         стиля строки — на десяти КП это самая дорогая статья прокрутки.
 *         Хук ставит на <table> `data-row-hover="off"` на время движения и
 *         снимает его ПЕРВЫМ ЖЕ ДВИЖЕНИЕМ КУРСОРА, а не по таймеру (почему
 *         именно так — у самих ворот в теле хука); правило подсветки в
 *         shared/ui/Table этим атрибутом и заперто (там же замеры и разбор
 *         формы селектора).
 *         Заодно это честнее по смыслу: подсветка отвечает на «где мой
 *         курсор», а во время прокрутки она отвечала на «мимо чего он
 *         пролетел».
 *
...
export function useTableDock({ focusRowId, onHeadStuckChange }: {
  ...
  useEffect(() => {
    ...
    /* ── ворота подсветки строки ───────────────────────────────────────────
        Атрибут ставится на саму <table>, а не на блок: только в одном
        компаунде с `.table` он отбрасывает правило целиком (разбор — в
        Table.module.css). Таблица за время жизни подписки не меняется —
        ищется один раз, как и скроллблок страницы.

        ЦЕНА ОДНОГО ПЕРЕКЛЮЧЕНИЯ — ПЕРЕСТРОЙКА СТИЛЯ ВСЕЙ ТАБЛИЦЫ, и из этого
        следует всё остальное. Ворота обязаны щёлкнуть РОВНО ДВА РАЗА за жест;
        каждое лишнее переключение сжигает ту же экономию, ради которой они
        заведены (числа и разбор — у самого правила в Table.module.css).

        ПОЭТОМУ ПОДСВЕТКУ ВОЗВРАЩАЕТ ДВИЖЕНИЕ КУРСОРА, А НЕ СОБЫТИЕ ВРЕМЕНИ.
        Две попытки сделать иначе замерены и отброшены:
        — таймер «140мс тишины»: под нагрузкой события колеса приходят реже
          собственного таймаута, атрибут зафлапал 14 раз на 60 событиях,
          пересчёт стилей 216 → 1531 мс;
        — нативный `scrollend`: он оказался концом не жеста, а каждой порции
          колеса — 37 переключений, 201 → 7604 мс.
        Курсор ведёт себя ровно наоборот: пока крутят колесо, он не двигается,
        поэтому ворота закрываются один раз и открываются один раз — на первом
        настоящем движении мыши. Заодно это и есть смысл подсветки: она
        отвечает на «где мой курсор», а не «мимо чего он пролетел».
        (Третья попавшаяся в ту же яму механика — `pointer-events: none` на
        время прокрутки, 23.08.2026, 77 → 1007 мс.) */
    const table = el.querySelector('table');
    const muteHover = () => {
      if (table && table.dataset.rowHover !== 'off') table.dataset.rowHover = 'off';
    };
    const unmuteHover = () => {
      if (table && table.dataset.rowHover === 'off') delete table.dataset.rowHover;
    };
    /* ПРОБОВАЛИ И ОТВЕРГЛИ: снимать ворота по `scrollend`, чтобы единственная
       инвалидация поддерева случалась в тихий момент, а не на первом движении
       курсора. Событие оказалось не «концом прокрутки», а концом КАЖДОЙ порции
       колеса: на 60 событиях выходило 37 переключений вместо одного, пересчёт
       стилей 201 → 7604 мс. Тот же класс ошибки, что и с таймером выше. */
    ...
    const onWheel = (e: WheelEvent) => {
      /* Колесо гасит подсветку и тогда, когда едет СТРАНИЦА: строки уходят
         из-под курсора одинаково, а `scroll` у ленты в этом случае не
         срабатывает вовсе. Вызов стоит ДО чтения rect'ов ниже сознательно:
         атрибут ставится один раз за жест, а форсированный пересчёт после
         него всё равно случится — пусть уж он будет тем самым единственным. */
      muteHover();
    ...
    el.addEventListener('pointermove', unmuteHover, { passive: true });
    return () => {
      el.removeEventListener('pointermove', unmuteHover);
