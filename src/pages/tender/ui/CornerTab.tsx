 ...
 * Каждая строка перечня — ПЕРЕХОД к своей ячейке в таблице.
 *
 ...
 *         раньше ответ на него был только у корректировок. Клик ведёт к первой
 *         такой ячейке колонки сверху вниз, повторный — к следующей; обход
 *         делается одной точкой, без списка.
...
export function CornerTab({ name, marks, onGoToMark }: {
  ...
  name: string;
  marks: ColumnMarks;
  /** Переход к первой ячейке колонки с этой пометкой. */
  ...
  const row = (kind: MarkKind, label: string, glyph?: ReactNode) => (marks[kind] ? (
    <button
      type="button"
      className={s.mark}
      aria-label={`${name}: ${label} — ${marks[kind]}. Перейти к первой такой ячейке`}
