...
export function Table({ caption, layout, stickyHead, stickyCol, width, children }: {
   ...
   *         рядовое значение.
   *
   * Требование к разметке: ячейка-ЯКОРЬ должна быть заголовком строки
   * (<th scope="row">), иначе якорь виден глазом, но не существует для
   * скринридера — а это ровно та же потеря, только в другом канале.
   *
   * ЗАКРЕПИТЬ МОЖНО ДВЕ ПЕРВЫЕ КОЛОНКИ (`stickyCol={2}`). Понадобилось это,
   * когда у сравнения КП появилась узкая колонка «№»: якорем осталось
   * название, а номер стоит ЛЕВЕЕ него и обязан ехать вместе с ним — иначе
   * закреплённое название отрывается от собственного номера. Смещение второй
   * колонки берётся из `--sticky-col-1` (ширину первой знает потребитель,
   * таблица её не считает); при `stickyCol={2}` ВТОРАЯ ячейка каждой строки
   * обязана существовать — colSpan-заглушка на её месте застыла бы у кромки
   * вместо якоря.
   */
  stickyCol?: boolean | 2;
  children: ReactNode;
