...
// Неполное КП в ранжире: первое неполное место — ПриволжскПромТехКомплект
// (2-е по итогу, 72 % заполнения). Его дыры: m4, w4 (нет цены) и g1 (отказ).
// Итог такого предложения занижен, и правило обязано назвать это до выбора.
...
assert.match(gapCard.title, /не закрыт у ООО «ПриволжскПромТехКомплект»/);
...
// ИнженерГрупп — 6-е место (последнее): пятеро неполных стоят впереди него
// по правилу «неполное вниз», и его итог — самый дорогой из шести.
assert.match(selectionCard.text, /1-е и 6-е места/);
