 ...
 *
 * КОГДА:  на странице тендера. Панель — колонка каркаса справа от <main>
 *         (слот в Workspace): её шапка стоит ровно на полосе .main-header
 *         страницы, и нижняя линия шапки чата продолжает нижнюю границу
 *         полосы крошек — линия одна.
 *         Открытость — проп open (гасит колонку слот каркаса); история
 *         разговоров живёт в состоянии компонента и умирает вместе со
 *         страницей тендера — осознанный долг, как у несохраняемых фильтров
 *         реестра; черновик ввода — тоже.
 ...
 *         и язычок на краю панели закрывают; фокус возвращает триггер сам, с
 *         preventScroll — закрытие не прокручивает страницу. Закрытая колонка
 *         гасится слотом каркаса (visibility:hidden) — табуляция в неё
 *         не попадает. Кнопки истории/нового чата — IconButton с именем;
