...
### Вкус проекта: три приоритета, в порядке убывания

При конфликте побеждает верхний.

**1. Логичный UX.** Экран сначала понятный, потом красивый. Состояния данных
проработаны всегда: загрузка (`<Skeleton>`/`<Spinner>`), ошибка с повтором
(`<ErrorState>`), пустота с выходом вперёд (`<EmptyState>`) — экран без этих
трёх состояний не закончен. Источник истины — URL; действие предсказуемо и
обратимо; значение, которое правят на месте, в покое выглядит текстом, а не
полем анкеты. Один смысл — один контрол: сетка месяца одна, окно и выпадашка
одни, статус описан одной доменной таблицей.

**2. Живое движение.** Интерфейс отвечает на каждое касание: ховеры по шкале
состояний, вход и выход панелей, раскрытие деталей, подсветка изменившегося.
Время и кривая — только токены: `--cu-duration-fast/base/slow`
(.12/.2/.28s) и `--cu-ease-standard/emphasized`; своих миллисекунд и
cubic-bezier не заводить. Движение обязано нести смысл, а не украшать:
причинность (меню выросло из кнопки), смену места, доменную механику —
штанги штангенциркуля `.spread-mark` на hover раскрываются, как инструмент,
это эталонный пример того, что приветствуется. Обязательная пара к любой
анимации — `@media (prefers-reduced-motion: reduce)` со статичным
эквивалентом сигнала: без движения сообщение остаётся, уходит только путь.

**3. Сдержанная оптика.** Никаких декораций без роли. Тень только
`--cu-elevation-*`, заливка покоя только `--cu-fill`, цвет никогда не
единственный канал смысла (у `<Badge>` всегда есть `icon`). Эффект «для
красоты» выкидывается; эффект «для смысла» остаётся.

### Иконки: один набор, одна толщина, одна точка входа

Набор — **Solar** (`@solar-icons/react`), стиль linear. Толщина линии 1.5
задаётся разово `<SolarProvider strokeWidth={1.5}>`; размер и вес глифа правит
CSS обёртки `.icon svg`, а не пропы компонента.

- Иконка берётся через `<Icon name="…">`, где `name` — id символа спрайта
  эталона. Таблица `shared/ui/Icon/icon-map.ts` сопоставила их с Solar **по
  геометрии путей**, потому что у Solar другие названия (`search→Magnifier`,
  `myTasks→Checklist`, `grid→Widget5`) — не подбирайте иконку по имени.
- Глифа нет в таблице → добавьте строку туда (так появились `closeCircle` и
  соседи): импорт из Solar плюс запись в `ICONS`. Своих импортов из
  `@solar-icons/react` вне этой таблицы не заводить.
- Самодельный SVG-путь — последний шаг, когда глифа нет и в Solar: таких
  четыре legacy-глифа рейла в `local-icons.tsx`, новых лучше не плодить.
- Один глиф не изображают другим: крестик повёрнутым `add` — приём из
  проекта убран, не возвращайте.
